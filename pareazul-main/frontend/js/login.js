...
function confirmarUsuario() {
  ...
  let validaSenhaIgual = false;

  if (senha == senha2) {
      validaSenhaIgual = true;
  }

  if (!validaSenhaIgual) {
      alert("Verifique: senha inválida!");
      return false;
  }
  ...
  callApiPost(
      ...
      function (data) {
          ...
          // perguntar para o professor, sobre o: + JSON.stringify(data)
          // se usar o json não gera o log

          console.log("Usuario gravado!" + JSON.stringify(data));

          // o console.log gera mensagem no F12, porém não pro usuário
          // console.log("Log: Usuario gravado!");
...
function confirmarAlteracaoSenha() {
  ...
  const senhanova = document.querySelector("#senha-nova-usuario").value;
  const senhanova2 = document.querySelector("#senha-nova-usuario2").value;
  ...
  if (!validaSenhaIgual) {
      alert("Verifique: nova senha não confere!");
      return false;
  }

  // validar login e senha atual
  const email = document.querySelector("#email-usuario").value;
  const senha = document.querySelector("#senha-atual-usuario").value;
  ...
  // para garantir que o usuario que esta alterando o senha, seja o mesmo usuario que está logado, e que sabe a senha
  callApiPost("POST", "login", function (data) {
      ...
      // Se chegou ate aqui, significa que a senha atual é valida e segue para a alteração de senha
      alterarSenhaUsuario();
      modalFecharConfirmacaoSenha();
  }, body);
}
...
function alterarSenhaUsuario() {
  ...
  const id_usuario_logado = document.querySelector("#usuario_logado").value;
  callApiPost("PUT", "senhausuario/" + id_usuario_logado,
      function (data) {
          console.log(data);
      }, body
  );
...
function modalFecharConfirmacaoSenha() {
  // modalFecharConfirmacaoSenha - Modal Alteração de Senha
